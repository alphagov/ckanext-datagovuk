...
from ckanext.harvest.logic.action.create import (
    harvest_job_create,
...
def create_dgu_test_data(context):
    ...
    if not all(
        os.environ.get(i)
        for i in ("CKAN_TEST_SYSADMIN_NAME", "CKAN_TEST_SYSADMIN_PASSWORD", "CKAN_INI")
    ):
        print(
            "One of these env vars not set: CKAN_INI, CKAN_TEST_SYSADMIN_NAME or CKAN_TEST_SYSADMIN_PASSWORD"
        )
        return
    ...
    if not publisher:
        ...
        category = model.GroupExtra(
            group_id=publisher.id, key="category", value="charity-ngo"
        )
        ...
        if not default:
            default = model.User(name="default", password="test1234")
        ...
        dataset = model.Package(
            name="test-dataset",
            title="Test dataset",
            notes="Test dataset for default user",
    ...
    if not model.Package.by_name("example-harvest-1"):
        ...
        if harvest_source_id:
            ...
            print(
                "=== Updating the example dataset to be in line with how DGU processes it"
            )
            dataset = model.Package.get("example-dataset-number-one")

            contact_name = model.PackageExtra(
                package_id=dataset.id, key="contact-name", value="Example User"
            )
            ...
            delete_fields = ["guid", "responsible-party", "taxonomy_url"]
            for key in delete_fields:
                field = (
                    model.Session.query(model.PackageExtra)
                    .filter(
                        model.PackageExtra.package_id == dataset.id,
                        model.PackageExtra.key == key,
                    )
                    .first()
                )
                if field:
    ...
    print(
        "=== To use with CKAN functional tests in ckan-vars.conf set OWNER_ORG=%s"
        % publisher.id
    )
...
def remove_dgu_test_data(context):
    ...
    model.Session.execute(
        sql, {"testadmin_name": os.environ.get("CKAN_TEST_SYSADMIN_NAME")}
    )
    model.repo.commit_and_remove()
...
def remove_ckan_admin(context):
    ...
    model.Session.execute(
        sql, {"ckan_admin_name": os.environ.get("CKAN_SYSADMIN_NAME")}
    )
    model.repo.commit_and_remove()
...
def reindex_recent(context):
    ...
    reindex_start = (
        datetime.now() - timedelta(minutes=REINDEX_MINUTES_BEFORE)
    ).strftime("%Y-%m-%d %H:%M")
    ...
    for i, package in enumerate(packages):
        ...
        print(
            f"{i + 1}/{len(packages)} - Updating search index:",
...
def reindex_organisations(context):
    ...
    existing_organisations = [
        r.get("name")
        for r in solr.search(
            "*", fq=f"(site_id:{DGU_ORGANISATIONS_ID})", rows=ROWS_TO_RETRIEVE
        )
    ]
    organisations = []
    counter = 0
