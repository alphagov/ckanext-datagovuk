...
def get_boundary(url):
    ...
    """
    if "statistical-geography" not in url:
        ...
        try:
            gss_url = (
                blob[0][
                    "http://opendatacommunities.org/def/local-government/governsGSS"
                ][0]["@id"]
                + ".json"
            )
        except KeyError:
    ...
    def chunk(l):  # noqa: E741  TODO: rename `l` — ambiguous name
        for i in xrange(0, len(l), 2):  # noqa: F821  TODO: `xrange` → `range` (Python 3)
            lat = l[i : i + 1][0]
            lng = l[i + 1 : i + 2][0]
